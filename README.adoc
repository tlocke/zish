...
specification of Zish] is an http://www.antlr.org/[ANTLR] grammar. The whitespace
characters between tokens are those with the Unicode category `Zs` ("Separator, space"),
or with the bidirectional class one of `WS`, `B`, or `S`. Zish has the following data
types:
