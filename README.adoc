...
representation of a real number in base 10. The exponent starts with a `e`.
Examples are:
...
separated by a `,` and the key is separated from the value with a `:`. Keys and
values can be of any Zish type. An example of a map is:
